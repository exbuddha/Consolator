...
internal var instance: BaseApplication? = null
internal var service: BaseService? = null
internal var receiver: BaseReceiver? = null
    get() = field.singleton().also { field = it }

internal var db: AppDatabase? = null
internal var logDb: LogDatabase? = null
internal var netDb: NetworkDatabase? = null
internal var session: RuntimeSessionEntity? = null
...
internal const val VIEW_MIN_DELAY = 300L
...
internal fun Context.stageAppDbCreated(scope: Any?) {
    // bootstrap
}

@Diverging([STAGE_BUILD_SESSION])
internal fun Context.stageSessionCreated(scope: Any?) {
    // update db records
}
...
internal suspend fun buildSession() {
    if (sessionIsNull)
        buildNewSession() }

internal suspend fun buildNewSession() {
    RuntimeDao {
        session = getSession(
            newSession(foregroundContext.startTime())) } }

internal fun Job.isSessionCreated() = sessionIsNotNull
...
internal suspend fun updateNetworkCapabilities(network: Network? = net.consolator.network, networkCapabilities: NetworkCapabilities? = net.consolator.networkCapabilities) {
...
internal fun <D : RoomDatabase> Context.createDatabase(cls: Class<D>, name: String?) =
    Room.databaseBuilder(this, cls, name).build()

internal fun <D : RoomDatabase> Context.createDatabase(cls: KClass<D>) =
...
internal fun Context.buildAppDatabase() =
    commitBuildDatabase(::db)

internal fun Job.isAppDbCreated() = appDbIsNotNull

internal fun String.toLocalTime() = dateTimeFormat!!.parse(this)!!.time

private fun Long.toLocalTimestamp() = dateTimeFormat!!.format(Date(this))

internal fun String.toAppTime() = toLocalTime() - dbTimeDiff!!

private fun Long.toDbTime() = plus(dbTimeDiff!!)

internal val appDbIsNull get() = db === null
internal val appDbIsNotNull get() = db !== null
internal val sessionIsNull get() = session === null
internal val sessionIsNotNull get() = session !== null
internal val logDbIsNull get() = logDb === null
internal val logDbIsNotNull get() = logDb !== null
internal val netDbIsNull get() = netDb === null
internal val netDbIsNotNull get() = netDb !== null
internal val appDbOrSessionIsNull get() = appDbIsNull || sessionIsNull
internal val logDbOrNetDbIsNull get() = logDbIsNull || netDbIsNull
...
internal val Context.isNetworkStateAccessPermitted
    get() = isPermissionGranted(ACCESS_NETWORK_STATE)

internal val Context.isInternetAccessPermitted
...
internal fun Intent?.makeClockStartSafely() = make {
    if (hasCategory(START_TIME_KEY))
        Clock.startSafely() }

internal inline fun <R> Intent?.make(change: Intent.() -> R) = this?.run(change)

internal interface ReferredContext { var ref: WeakContext? }

internal typealias WeakContext = WeakReference<out Context>

internal fun Context.asWeakReference() =
    if (this is ReferredContext) ref!!
    else WeakReference(this)

internal fun <T : Context> WeakReference<out T>?.unique(context: T) =
    require { WeakReference(context) }

internal interface UniqueContext { var startTime: Long }

internal fun Context.startTime() = asUniqueContext()?.startTime ?: -1L

internal fun now() = Calendar.getInstance().timeInMillis
...
internal inline fun <R> tryCanceling(block: () -> R) =
...
internal suspend inline fun <R> tryCancelingSuspended(crossinline block: suspend () -> R) = tryCanceling { block() }
internal suspend inline fun <T, R> tryCancelingSuspended(scope: T, crossinline block: suspend T.() -> R) = tryCanceling { scope.block() }
internal suspend inline fun <T, R> tryCancelingSuspended(crossinline scope: suspend () -> T, crossinline block: suspend T.() -> R) = tryCancelingSuspended(scope(), block)
...
internal fun Any?.asObjectProvider() = asType<ObjectProvider>()
...
internal typealias ObjectProvider = (AnyKClass) -> Any
...
internal lateinit var mainUncaughtExceptionHandler: ExceptionHandler

internal open class BaseImplementationRestriction(
...
internal var log = fun(log: LogFunction, tag: String, msg: String) { if (log.isOn) log(tag, msg) }

internal var info: LogFunction = fun(tag: String, msg: String) = Log.i(tag, msg)
internal var debug: LogFunction = fun(tag: String, msg: String) = Log.d(tag, msg)
internal var warning: LogFunction = fun(tag: String, msg: String) = Log.w(tag, msg)
private val bypass: LogFunction = { _, _ -> }

internal val LogFunction.isOn
    get() = this !== bypass
internal val LogFunction.isOff
    get() = this === bypass
...
internal const val EXCEPTION = "exception"
internal const val CAUSE = "cause"
internal const val MESSAGE = "message"
internal const val EXCEPTION_CAUSE = "$EXCEPTION-$CAUSE"
internal const val EXCEPTION_MESSAGE = "$EXCEPTION-$MESSAGE"
internal const val EXCEPTION_CAUSE_MESSAGE = "$EXCEPTION-$CAUSE-$MESSAGE"
internal const val IGNORE = "ignore"
internal const val UNCAUGHT = "uncaught"
...
internal const val VIEW_ATTACH = "$VIEW.$ATTACH"
...
internal const val UNCAUGHT_SHARED = "$UNCAUGHT-$SHARED"

internal const val STAGE_BUILD_APP_DB = "$APP_DB.$BUILD"
internal const val STAGE_BUILD_SESSION = "$SESSION.$BUILD"
